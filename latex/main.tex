...
\begin{frame}{Asset screening}
  \begin{itemize}
    \item Five cryptocurrencies: balance between market cap (Nov. 2025) and 24h trading volume (\href{https://coinmarketcap.com/}{CoinMarketCap})
    \item Exclude meme coins (e.g., DOGE, SHIB), stablecoins (e.g., USDT, USDC).
    \item Relatively mature infrastructure and ecosystem.
  \end{itemize}
  \ \\
  \ \\
  \begin{enumerate}
...
\begin{frame}{Data collection}
  \begin{itemize}
    \item CoinMarketCap through Yahoo Finance API.
    \begin{itemize}
      ...
      \item Data quality: outliers due to liquidity squeeze and exchange specific issues during market turmoil.
    \end{itemize} 
...
\end{table}
\begin{itemize}
  \item All-in-BTC (annualized): expected return$=60.66\%$, std. dev.$=56.37\%$, Sharpe ratio$=1.076$, max drawdown$=-70\%$.
\end{itemize}
\end{frame}
...
\begin{frame}{Performance assessment: simulation}
\begin{itemize}
  ...
  \begin{itemize}
    \item Typically used to reconstruct option implied volatility surface.
    \item Fat tails
    \item Volatility clustering
    \item Mean-reverting volatility
    \item Price-volatility co-movement.
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{Epilogue \& possible extensions}
  \begin{itemize}
    \item GenAI? Alpha Arena: \url{https://nof1.ai/}
    \begin{itemize}
      \item Day trading/short-term (hours/days) holding period.
      \item Could possibly shed light on long-term asset allocation decisions.
    \end{itemize} 
  \end{itemize}
  \begin{figure}[htbp] % h=here, t=top, b=bottom, p=page
    \centering
    \includegraphics[width=\textwidth]{figure/Figure_9.png}
    \label{fig:figure8}
\end{figure}
\end{frame}
...
\begin{frame}{Epilogue \& possible extensions}
    ...
    \item Beyond just the state-space: 2D state-action space dynamic optimization (reinforcement learning).
    \begin{itemize}
      \item state: economic regime, market conditions, etc.
      \item action: allocation decision.
    \end{itemize}
  \end{itemize}
\end{frame}
